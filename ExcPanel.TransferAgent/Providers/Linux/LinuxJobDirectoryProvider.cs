...
public class LinuxJobDirectoryProvider : IJobDirectoryProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return JobDirectoryOperationResult<JobDirectoryInfoResponse>.Ok(EnrichWithUncPaths(new JobDirectoryInfoResponse
        {
            JobId = jobId.ToString("D"),
            JobType = jobType,
        ...
        }, jobId, jobType, relativePath, physicalPath));
    }
    ...
    public bool TryResolveJobDirectory(
        Guid jobId,
        JobDirectoryType jobType,
    ...
    private static JobDirectoryCreateResponse BuildCreateResponse(
        ...
        SambaPathService sambaPathService)
    {
        var response = new JobDirectoryCreateResponse
        {
            JobId = jobId.ToString("D"),
            JobType = jobType,
            RelativePath = relativePath,
        ...
        ApplyUncPaths(response, jobId, jobType, relativePath, physicalPath, sambaPathService);
        return response;
    }

    private JobDirectoryCreateResponse BuildCreateResponse(
        Guid jobId,
        JobDirectoryType jobType,
        string relativePath,
        string physicalPath,
        bool created) =>
        BuildCreateResponse(jobId, jobType, relativePath, physicalPath, created, _sambaPathService);

    private JobDirectoryInfoResponse EnrichWithUncPaths(
        JobDirectoryInfoResponse response,
        Guid jobId,
        JobDirectoryType jobType,
        string relativePath,
        string physicalPath)
    {
        ApplyUncPaths(response, jobId, jobType, relativePath, physicalPath, _sambaPathService);
        return response;
    }

    private static void ApplyUncPaths(
        JobDirectoryCreateResponse response,
        Guid jobId,
        JobDirectoryType jobType,
        string relativePath,
        string physicalPath,
        SambaPathService sambaPathService)
    {
        var unc = ResolveUncPaths(jobId, jobType, relativePath, physicalPath, sambaPathService);
        if (unc is null)
        {
            return;
        }

        response.UncDirectory = unc.UncDirectory;
        response.SuggestedFileName = unc.SuggestedFileName;
        response.UncFilePath = unc.UncFilePath;
    }
    ...
    private static SambaUncPathResponse? ResolveUncPaths(
        Guid jobId,
        JobDirectoryType jobType,
        string relativePath,
        string physicalPath,
        SambaPathService sambaPathService) =>
        sambaPathService.TryBuildUncPath(jobId, jobType, relativePath, physicalPath);
