...
{
    internal static Func<IEnumerable<string>>? MountInfoReaderOverride { get; set; }
    ...
    public static bool IsBindMountActive(string filesMountPath, string expectedJobPath)
    {
        ...
        if (!Directory.Exists(expectedJobPath))
        {
            return false;
        }
    ...
    public static bool IsMountPoint(string mountPath)
    {
        if (!OperatingSystem.IsLinux())
        {
            return false;
        ...
        var normalizedMountPoint = NormalizePath(mountPath);
        return ReadMountInfoLines().Any(line =>
            TryParseMountInfoLine(line, out _, out var mountPoint, out _)
            && string.Equals(mountPoint, normalizedMountPoint, StringComparison.Ordinal));
    }
    ...
    private static string? ResolveJobPathFromBindRoot(string root, string sourceDevice)
    {
        ...
        foreach (var line in ReadMountInfoLines())
        {
            ...
            var candidate = NormalizePath(Path.Combine(mountPoint, normalizedRoot));
    ...
    {
        if (MountInfoReaderOverride is not null)
        {
            foreach (var line in MountInfoReaderOverride())
            {
                yield return line;
            }
        ...
        if (!File.Exists(SftpHostMountHelper.HostMountInfoPath))
        {
            yield break;
        }

        foreach (var line in File.ReadLines(SftpHostMountHelper.HostMountInfoPath))
        {
            yield return line;
        }
    }
